...
var RingHttpServer = module.exports = function RingHttpServer(options) {
	...
	// handle options
	options || (options = {});
	options.name || (options.name = self.ringFamily + '-ring-http-server');
...
RingHttpServer.prototype.init = function(done) {
	...
	self.server = http.createServer(self.getRequestListener());
	self.bindServerToFirstAvailablePort(function(err, port) {
		if (err) return done(err);
		var configValues = { address: self.guessAddress() };//self.getDefaultConfigValues();
...
RingHttpServer.prototype.bindServerToFirstAvailablePort = function(cb) {
	...
	self.server.on('error', onListenError);
	self.server.on('listening', function() {
		// on future errors we just want to emit them at the hook level
		self.server.removeListener('error', onListenError);
